class BaseAccessory {
    ...
    convertColorTemperatureFromHomeKitToTuya(value) {
        if (this.device.context.linearWhiteColor) {
            ...
            return Math.min(scale, Math.max(0, Math.round((scale * min / (max - min)) * ((max / value) - 1))));
        } else {
            const min = this.device.context.minWhiteColor || 140;
            const max = this.device.context.maxWhiteColor || 400;
            const scale = this.device.context.scaleWhiteColor || 255;
    ...
    convertColorTemperatureFromTuyaToHomeKit(value) {
        if (this.device.context.linearWhiteColor) {
            ...
            const max = this.device.context.maxWhiteColor || 400;
            const scale = this.device.context.scaleWhiteColor || 255;
            const adjustedValue = (max / value) - 1;
            return Math.min(scale, Math.max(0, Math.round((scale * min / (max - min)) * adjustedValue)));
        } else {
            const min = this.device.context.minWhiteColor || 140;
            const max = this.device.context.maxWhiteColor || 400;
            const scale = this.device.context.scaleWhiteColor || 255;
